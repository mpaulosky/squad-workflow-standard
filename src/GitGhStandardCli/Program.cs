...
internal static class Program
{
    public static int Main(string[] args)
    {
        ...
        if (command == "sync-mesh")
        {
            ...
            {
                Console.Error.WriteLine(
                    "Unable to locate repository root (.git). Run this command from within the repository.");
                return 1;
            }
            ...
            if (isWindows)
            {
                var scriptPath = Path.Combine(gitRepoRoot, ".squad", "templates", "skills", "distributed-mesh",
                    "sync-mesh.ps1");
                fileName = "pwsh";
            ...
            {
                fileName = "bash";
                scriptArgs.Add(Path.Combine(gitRepoRoot, ".squad", "templates", "skills", "distributed-mesh",
                    "sync-mesh.sh"));
    ...
    /// </summary>
    private static string? FindSourceRepoRoot(string start)
    {
    ...
    private static string? FindGitRepoRoot(string start)
    {
        var current = new DirectoryInfo(Path.GetFullPath(start));
        while (current is not null)
        {
    ...
    /// </summary>
    private static bool ParseSyncCheckArgs(
        string[] remaining,
