...
1. Defines the canonical workflow policy (branch model, PR flow, hard gates, cleanup).
2. Distributes policy assets into target repositories.
3. Validates drift and enforcement wiring in target repositories.
4. Ships a centrally managed GitHub Actions baseline for squad automation/linting/release ceremonies.

## Project structure

| Path | Purpose |
| --- | --- |
| `.squad/workflows/git-gh-process-standard.md` | Canonical process standard (`Standard-Version` source of truth). |
| `.squad/workflows/README.md` | Rollout/retrofit playbook for adopting the standard in target repos. |
| `.squad/workflows/workflow-baseline-manifest.txt` | List of centrally managed workflow files to sync into target repos. |
| `.squad/skills/git-workflow-standard/SKILL.md` | Skill-facing executable guidance bound to the same standard version. |
| `scripts/squad/sync-git-gh-standard.sh` | Syncs standard assets + baseline workflows into a target repo. |
| `scripts/squad/check-git-gh-standard.sh` | Checks version drift and required adapter/enforcement bindings. |
| `.github/workflows/*.yml` | Reusable baseline workflows (CI/linting/labeling/triage/release/project-board/blog automation). |
| `.github/hooks/pre-commit` | Markdown lint gate for staged `.md` files. |
| `.github/hooks/pre-push` | Strict pre-push gate script template used in participating repos. |
...
```bash
bash scripts/squad/sync-git-gh-standard.sh /absolute/path/to/target-repo --source-repo /absolute/path/to/canonical-repo
```
...
YAML linting is enforced in CI via `squad-lint-yaml.yml` (yamllint rules embedded in workflow config).
